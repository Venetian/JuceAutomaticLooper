...
JuceMidiFilePlayer::JuceMidiFilePlayer(){
    ...
    looper.setLoopPointsBeats(0, 8);
    prophet.setLoopPointsBeats(0, 8);
...
MidiMessageSequence JuceMidiFilePlayer::loadMidiFile(String fileLocation, bool mergeOn){
    ...
    if (!file.isDirectory())
    {
       ...
       // loopSequence.clear();
        ...
        //set all channel one
        for (int i = 0; i < loadedSequence.getNumEvents(); i++){
            loadedSequence.getEventPointer(i)->message.setChannel(1);
        }
